# === EDUCATIONAL CRYPTO SIGNAL BOT — Sniper Futures-Only Fast Breakdown (v4: AI News Filter + Telegram Status) ===
...
# === AI News Filter (OpenAI) ===
def classify_news_ai(title: str):
    """Return True if news is critical enough to pause trades"""
    try:
...
def fast_breakdown_loop():
    ...
    while True:
        try:
            ...
            session_text = "🌍 Peak Hours (London/NY)" if session_active else "🌙 Off-Hours (Asia/Low Volume)"
                    ...
                    if direction:
                        ...
                        if AUTO_TRADE_ENABLED and check_daily_loss():
                            try:
                                ...
                                open_trades[sym] = {"entry": entry, "stop": stop, "target": target, "contracts": contracts, "bias": direction, "atr": atr, "partial_exit": False}
            ...
            for sym, trade in list(open_trades.items()):
                ...
                if trade['partial_exit']:
                    if trade['bias'] == "LONG" and price >= trade['entry'] + TRAILING_ACTIVATE * trade['atr']:
                        trade['stop'] = max(trade['stop'], trade['entry'])
                    elif trade['bias'] == "SHORT" and price <= trade['entry'] - TRAILING_ACTIVATE * trade['atr']:
...
if __name__ == '__main__':
    send_telegram("✅ Sniper Bot Started (v4: AI News Filter + Status Alerts)")
